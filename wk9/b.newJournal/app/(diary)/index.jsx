...
export default function DiaryHomeScreen() {
  ...
  const activeJournalTitle = useDiaryStore((s) => {
    const j = s.journals.find((x) => x.id === s.activeJournalId);
    return j?.name ?? '日誌';
  });

  /** 日記列表頁頂部標題：與目前選中的「日誌」名稱一致（根 Stack 上 (diary) 的 header） */
  useLayoutEffect(() => {
    rootNavigation.setOptions({ title: activeJournalTitle });
  }, [rootNavigation, activeJournalTitle]);
