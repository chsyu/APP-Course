...
import { syncDiariesBidirectional } from '../services/diaryService';
...
export default function SyncScreen() {
  ...
  const { diaries, mergeDiaries } = useDiaryStore();
  ...
  const handleSyncPress = async () => {
    ...
    try {
      // 雙向同步日記（下載雲端資料並合併，然後上傳本地更新的資料）
      const result = await syncDiariesBidirectional(user.uid, diaries);

      if (result.success) {
        // 合併雲端資料到本地
        if (result.mergedDiaries && result.mergedDiaries.length > 0) {
          mergeDiaries(result.mergedDiaries);
        }
        ...
        if (result.downloadedCount > 0) {
          parts.push(`下載 ${result.downloadedCount} 篇`);
        }
        if (result.syncedCount > 0) {
          parts.push(`新建 ${result.syncedCount} 篇`);
        }
        if (result.updatedCount > 0) {
          parts.push(`更新 ${result.updatedCount} 篇`);
        }
        if (result.skippedCount > 0) {
          parts.push(`跳過 ${result.skippedCount} 篇（已是最新）`);
        }
