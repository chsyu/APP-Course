...
import { fetchOpenWeatherByCoordinates } from '../services/openWeatherService';
import { getDiaryCreationCoordinates } from '../utils/locationHelper';
  ...
  persist(
    (set, get) => ({
          ...
          for (const r of remoteList) {
            ...
            if (!cur || remoteMs > diaryUpdatedMs(cur)) {
              byId.set(r.id, {
                ...
                ...(r.weatherAtCreate &&
                typeof r.weatherAtCreate === 'object' &&
                typeof r.weatherAtCreate.condition === 'string' &&
                typeof r.weatherAtCreate.value === 'string'
                  ? {
                      weatherAtCreate: {
                        condition: r.weatherAtCreate.condition,
                        value: r.weatherAtCreate.value,
                        ...(typeof r.weatherAtCreate.periodStart === 'string'
                          ? { periodStart: r.weatherAtCreate.periodStart }
                          : {}),
                      },
                    }
                  : {}),
      ...
      createDiary: async (title = '新日記', content = '') => {
        ...
        const coords = await getDiaryCreationCoordinates();
        const weatherAtCreate = await fetchOpenWeatherByCoordinates(
          coords.latitude,
          coords.longitude
        );
